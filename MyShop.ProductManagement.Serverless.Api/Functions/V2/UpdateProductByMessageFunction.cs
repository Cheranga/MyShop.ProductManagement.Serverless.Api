...
{
    public class UpdateProductByMessageFunction
    {
        private readonly IMediator _mediator;
        private readonly IMessageReader _messageReader;

        public UpdateProductByMessageFunction(IMediator mediator, IMessageReader messageReader)
        {
            _mediator = mediator;
            _messageReader = messageReader;
        }

        [FunctionName(nameof(UpdateProductByMessageFunction))]
        public async Task Run(
            [ServiceBusTrigger("%ServiceBusConfig:WriteTopic%", "%ServiceBusConfig:UpdateProductSubscription%", Connection = "ServiceBusConnection", IsSessionsEnabled = true)]
            Message message,
